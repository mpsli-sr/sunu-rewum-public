...
export default function Page() {
  ...
  useEffect(() => {
    http
      ...
      .catch(() => setUser(null));
    http
      .get<any>("/api/ideology/souverainete-alimentaire")
      .then((p) => {
        if (p) {
          setPage(p);
      ...
      .catch(() =>
        setPage({
          title: "souverainete-alimentaire",
  ...
  const save = async () => {
    try {
      await http.put("/api/ideology/souverainete-alimentaire", {
        title: page.title,
        content: editContent,
      });
  ...
  return (
    <div>
      ...
      <EditableBlockRenderer page="souverainete-alimentaire" />
    </div>
  );
}
